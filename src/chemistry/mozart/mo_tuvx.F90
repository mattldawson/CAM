...
!================================================================================================
  ...
  subroutine set_heights( this, i_col, ncol, height_mid, height_int )
    ...
    edges(pver+2) = edges(pver+1) + 1.0e-10 ! create a thin layer at the top to hold exo values
    ...
    mid_points(pver+1) = edges(pver+1) + ( edges(pver+2) - edges(pver+1) ) * 0.5_r8
...
!================================================================================================
  ...
  subroutine get_aerosol_optical_properties( this, state, pbuf )
    ...
    do i_col = 1, pcols
      ...
      ! use upper mid-point values for top edge
      call rebin(nswbands, n_tuvx_bins, wavelength_edges, this%wavelength_edges_, &
                 aer_tau(i_col,0,:), this%optical_depth_(i_col,pver+1,:))
      call rebin(nswbands, n_tuvx_bins, wavelength_edges, this%wavelength_edges_, &
                 aer_tau_w(i_col,0,:), this%single_scattering_albedo_(i_col,pver+1,:))
      call rebin(nswbands, n_tuvx_bins, wavelength_edges, this%wavelength_edges_, &
                 aer_tau_w_g(i_col,0,:), this%asymmetry_factor_(i_col,pver+1,:))
    end do
